...
import { ImageIcon, CopyIcon, ArrowSquareOutIcon } from '@phosphor-icons/react'
...
function GalleryCard({ image }: GalleryCardProps) {
  ...
  const handleOpen = useCallback(() => {
    window.api.file.openItem(image.path)
  }, [image.path])

  const handleCopy = useCallback(async () => {
    try {
      const uint8 = await window.api.file.readFile(image.path)
      const ext = image.name.split('.').pop()?.toLowerCase() ?? ''
      const type = MIME_TYPES[ext] ?? 'application/octet-stream'
      const blob = new Blob([uint8.slice().buffer], { type })
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        ...
        >
          ...
          <Badge
            variant="secondary"
            className="absolute top-0 right-0 shrink-0 text-xs bg-background"
          >
